...
def list_reservations(property_id: int = Query(None, description="Informe o ID da propriedade para listar as reservas"),
                      property_email: str = Query(None, description="Informe o email do cliente para listar as reservas"),
    ...
    if property_email:
        reservations_list = db.query(Reservation).filter(Reservation.client_email == property_email).all()
        return reservations_list
